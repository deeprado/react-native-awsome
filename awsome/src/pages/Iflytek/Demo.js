...
export default class App extends Component {
  ...
  componentDidMount() {
    ...
    this.recognizerEventEmitter.addListener(
      'onRecognizerError',
      this.onRecognizerError,
    );
    this.recognizerEventEmitter.addListener(
      'onRecognizerVolumeChanged',
      this.onRecognizerVolumeChanged,
    );

    this.synthesizerEventEmitter = new NativeEventEmitter(Synthesizer);
    this.synthesizerEventEmitter.addListener(
      'onSynthesizerSpeakCompletedEvent',
      this.onSynthesizerSpeakCompletedEvent,
    );
    this.synthesizerEventEmitter.addListener(
      'onSynthesizerBufferCompletedEvent',
      this.onSynthesizerBufferCompletedEvent,
    );
  }
    ...
    return (
      <View style={styles.container} onStartShouldSetResponder={() => true}>
        ...
        <Button
          containerStyle={styles.containerStyle}
          style={{color: 'white'}}
          onPress={this.onIsSpeakingBtnPress}
          activeOpacity={0.8}>
          is Speaking?
        </Button>
        <Button
