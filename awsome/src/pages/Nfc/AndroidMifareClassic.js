...
class App extends Component {
            ...
            <TouchableOpacity
              ...
              <Text style={{color: 'blue', textAlign: 'center', fontSize: 20}}>
                {`CLICK TO START DETECTING ${
                  this.state.mode === 'read' ? 'READ' : 'WRITE'
                }`}
              </Text>
            </TouchableOpacity>
          )}
  ...
  _startDetection = () => {
    ...
    const read = () => {
      return NfcManager.mifareClassicGetBlockCountInSector(
    ...
    const write = () => {
      return NfcManager.mifareClassicSectorToBlock(parseInt(this.state.sector))
        .then(block => {
          ...
          for (
            let i = 0;
            i < this.state.textToWrite.length &&
            i < NfcManager.MIFARE_BLOCK_SIZE;
            i++
          ) {
